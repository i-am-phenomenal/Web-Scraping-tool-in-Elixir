  ...
  def get_basic_movie_details(movie_url) do
    ...
    case HTTPoison.get(movie_url) do
      {:ok, %HTTPoison.Response{status_code: 200, body: body}} ->
        ...
        director_name =
          if extracted_titles |> Enum.any?(fn value -> value == "Director:" end) do
        ...
        writers_name =
          if extracted_titles |> Enum.any?(fn value -> value == "Writer:" end) do
            extract_writers_name(floki_response)
          else
            ""
          end

        movie_cast =
          if extracted_titles |> Enum.any?(fn value -> value == "Stars:" end) do
            floki_response
            |> Enum.at(2)
            |> elem(2)
            |> Enum.map(fn resp ->
