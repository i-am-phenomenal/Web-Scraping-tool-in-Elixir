defmodule WmDevForumWeb.PageController do
  ...
  def get_clicked_movie_details(conn, params) do
    ...
    basic_details =
      UserManagement.get_basic_movie_details(movie_url) |> IO.inspect(label: "DETAILS FETCHED ")

    # extended_details =
    #   UserManagement.get_extended_details(movie_url)

    updated_details_map =
      basic_details
      |> Map.put(:movie_name, params |> Map.get("movie_name") |> String.trim())

    render(conn, "movie_details.html", basic_details: updated_details_map)
  end
